...
const AddPost = (props) => {
    ...
    // title=변수명, setTitle=해당 변수를 갱신해주는 함수
    // title이라는 state변수와 해당 state를 갱신할 수 있는 함수 setTitle이 만들어졌음
    // useState인자값으로 _post라면 _post.title불러오고 아니라면 ""
    const onChangeTitle = useCallback((e) => {
        setTitle(e.target.value);
      }, []) // callback함수 이부분 신경안써도됨
    const [content, setContent] = React.useState(_post? _post.comment : ""); //초기값으로 _post.word값 설정
    const onChangeContent = useCallback((e) => {
        setContent(e.target.value);
      }, [])
    const [author, setAuthor] = React.useState(_post? _post.author : ""); //초기값으로 _post.word값 설정
